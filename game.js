...
function hitBox(x1, x2, hitBoxX, y1, y2, hitBoxY){
  if (Math.abs(distance(x1, x2)) <= hitBoxX && Math.abs(distance(y1, y2)) <=hitBoxY){
    return true;
  }
  return false;
}
...
function laserColapse(){
  for(var i=0; i<aliens.length; i++){
    if(hitBox(ship.laserX, aliens[i].laserX, 10, ship.laserY, aliens[i].laserY, 50) && aliens[i].shooting && ship.shooting){
      ship.stopShoot();
      aliens[i].stopShoot();
    }if(hitBox(ship.laserX, aliens[i].x, 30, ship.laserY, aliens[i].y, 50) && aliens[i].alive){
...
// loop
var imgsToHide = document.querySelectorAll('#top-container img');
var playButton = document.querySelector('#playButton');
playButton.addEventListener("click", function(){
  document.querySelector("#top-container h1").innerHTML = "use 'a' and 'd' or '←' and '→' to move and 'q' to shoot"
  for(var i=0; i<imgsToHide.length; i++){
    imgsToHide[i].classList.add("img-to-hide");
  }
  playButton.disabled = true
  playButton.classList.add("text-to-hide");
  var FPS = 20;
